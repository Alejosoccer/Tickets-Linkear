        ...
        <div class="row">
            <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="section-headline text-center">
                    <h2 style="margin-top: 50px;">Seleciona tu asiento</h2>
                </div>
            </div>
        </div>
        <div class="row">
            ...
            <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="col-md-4 col-sm-4 col-xs-12">
                    <button class="volver-atras" type="button" id="btn-default"><span id="number">1</span> SELECIONA LOS BOLETOS A COMPRAR</button>
                </div>

                <div class="col-md-4 col-sm-4 col-xs-12">
                    <button type="button" id="btn-default" disabled><span id="number">2</span>&nbsp; FACTURACIÓN Y VALIDACIÓN </button>
                </div>
            ...
            <div class="col-md-12 col-sm-12 col-xs-12" id="seleccionar-asiento">

                
                    <div class="col-md-3 col-sm-3">
                        
                        <p class="text-uppercase"><i class="fa fa-bus bybus" style="font-size: 30px;"></i> <b>Cooperativa:</b></p>
                        <p class="text-uppercase"><i class="fa fa-print" style="font-size: 30px;"></i> <b>Bus N°:</b></p>
                        <p class="text-uppercase"><i class="fa fa-map-marker" style="font-size: 30px;"></i><b>Ruta de Viaje:</b></p>
                        <p class="text-uppercase"><i class="fa fa-calendar" style="font-size: 25px;"></i> <b>Fecha de viaje:</b></p>
                    </div>

                    <div class="col-md-9 col-sm-9" style="margin-top: 10px;" id="cooperativa_nac">
                        @foreach ($query as $item)
                        <p class="text-uppercase"><i class="" style="font-size: 30px;"></i> <b>{{$item->empresa}}</b></p>
                        <p class="text-uppercase"><i class="" style="font-size: 30px;"></i> <b>{{$item->bus}}</b></p>
                        <p class="text-uppercase"><i class="" style="font-size: 30px;"></i><b>{{$item->origen}} a {{$item->destino}}</b></p>
                        <p class="text-uppercase"><i class="" style="font-size: 25px;"></i> <b>{{$item->fecha}}</b></p>
                        @endforeach
                    </div>
            ...
            <div class="container">
                ...
                <div class="section-headline text-center">
                    @foreach ($query as $item)
                    <h2 style="margin-top: 100px;">Bus N°{{$item->bus}}</h2>
                    @endforeach
                </div>
                <h3 class="text-left">PISO N°1 </h3>

                <span class="libre">Libre&nbsp;<i id="no_reservado" class="fa fa-circle"></i></span> <span id="separador">ESTADO</span>
                <span class="libre">Reservado&nbsp;<i id="reservado"class="fa fa-circle"></i></span>
                
            </div>

            <div class="col-md-6 col-sm-6 col-xs-12" id="asiento">

                @foreach ($bus as $bus)
                @if($bus->estado === 'N')
                <div class="col-md-3 col-sm-2 col-xs-12 fila">
                    <button  id="bus_select" class="btn btn-default dev asientos_select" type="button" name="{{$bus->id}}" onclick="enviarDatos(this)" disabled><span><img src="https://cdnx.jumpseller.com/vapormax1/image/3664671/resize/540/540?1528434986" alt=""><br> N°{{$bus->asiento}}</span></button>
                    </div>
                @else
                
                <div class="col-md-3 col-sm-2 col-xs-12 fila">
                <button id="bus_select" class="btn btn-default dev" type="button" name="{{$bus->id}}" onclick="enviarDatos(this)"><span><img src="https://cdnx.jumpseller.com/vapormax1/image/3664671/resize/540/540?1528434986" alt=""><br> N°{{$bus->asiento}}</span></button>
                </div>
                @endif
                @endforeach
            ...
            <form action="{{url('reservar')}}" method="POST">
                    ...
                    <input type="hidden" name="reserva[]" id="reserva_input" value="">
                    <input type="hidden" name="admin_user" value={{Auth::user()->id}}>

                    <div class="col-md-12 col-sm-12">
                        <button type="submit" class="form-btn" onclick="enviarReservaItem()">Agregar carrito</button>
                    </div>
                </form>
...
<!-- End About area -->
    ...
    <a href="{{url('home')}}" class="back-to-top"><i class="fa fa-undo"></i></a>
        ...
        <script>
            ...
            var todosBotones = document.querySelectorAll('.dev');
                
                    todosBotones.forEach(botones => 

                    botones.addEventListener('click', function(params) { 

                    botones.classList.toggle("asientos_select");
                ...
                function enviarDatos(params){
                    ...
                    var reserva = params.name;
                    arrayJson.push(reserva);

                    $('#reserva_input').val(arrayJson);
